...
public:
    ...
    // editor, recompiles it, and shows a short load summary -- the CEL
    // equivalent of Patina's showLoadedArtifactSummary, but there's no
    // separate compiled-artifact format to summarize: CEL source is
    // re-parsed on load exactly like it is on every edit.
