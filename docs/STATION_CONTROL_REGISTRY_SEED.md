...
Concrete `set_state`/`get_state` entries (no FRust-side spec for this
mechanism exists yet, so this doc's state-vs-action split is carried
forward as intent only until that's established) for Creation
