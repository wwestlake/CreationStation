...
Resolved: the bespoke `Patina` DSL (lexer/parser/lowering/artifact format) and the later CEL frontend that replaced it have both been removed outright. Station consumes FRust (third_party/FrustLang) directly — no app-local language implementation, no host-dialect decision to make. Remaining steps:
