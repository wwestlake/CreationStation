...
#define OFFLOAD_FILE_PATH           L"\\DriverData\\Audio_Samples\\Sysvad\\STREAM_OFFLOAD.wav"
#define HOST_FILE_PATH              L"\\DriverData\\Audio_Samples\\Sysvad\\STREAM_HOST.wav"
...
)
{
    ...
    NTSTATUS            ntStatus = STATUS_SUCCESS;
    PCWSTR              filePath = _bOffloaded ? OFFLOAD_FILE_PATH : HOST_FILE_PATH;
    size_t              cLen = 0;
    ...
    {
        m_FileName.Length = 0;
        ntStatus = RtlStringCchLengthW(filePath, NTSTRSAFE_MAX_CCH, &cLen);
    }
    ...
    if (NT_SUCCESS(ntStatus))
    {
        RtlStringCbCopyW(m_FileName.Buffer, m_FileName.MaximumLength, filePath);
