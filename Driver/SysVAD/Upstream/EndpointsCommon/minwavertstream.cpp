...
#define DJEHUTI_ROUTER_CABLE_BUFFER_BYTES 15360

typedef struct _DJEHUTI_ROUTER_CABLE_BUFFER
{
    KSPIN_LOCK Lock;
    ...
    ULONG ReadCursor;
    ULONG BufferedBytes;
    BYTE Data[DJEHUTI_ROUTER_CABLE_BUFFER_BYTES];
} DJEHUTI_ROUTER_CABLE_BUFFER;

static DJEHUTI_ROUTER_CABLE_BUFFER g_DjehutiRouterCableBuffer = {};

static
VOID
DjehutiRouterEnsureCableBufferInitialized()
{
    if (InterlockedCompareExchange(&g_DjehutiRouterCableBuffer.Initialized, 1, 0) == 0)
    {
        KeInitializeSpinLock(&g_DjehutiRouterCableBuffer.Lock);
        g_DjehutiRouterCableBuffer.WriteCursor = 0;
        g_DjehutiRouterCableBuffer.ReadCursor = 0;
        g_DjehutiRouterCableBuffer.BufferedBytes = 0;
        RtlZeroMemory(g_DjehutiRouterCableBuffer.Data, sizeof(g_DjehutiRouterCableBuffer.Data));
    }
}

static
VOID
DjehutiRouterCableWriteBytes
(
...
)
{
    ...
    DjehutiRouterEnsureCableBufferInitialized();
    KeAcquireSpinLock(&g_DjehutiRouterCableBuffer.Lock, &oldIrql);

    for (ULONG index = 0; index < ByteCount; ++index)
    {
        g_DjehutiRouterCableBuffer.Data[g_DjehutiRouterCableBuffer.WriteCursor] = Source[index];
        g_DjehutiRouterCableBuffer.WriteCursor =
            (g_DjehutiRouterCableBuffer.WriteCursor + 1) % DJEHUTI_ROUTER_CABLE_BUFFER_BYTES;

        if (g_DjehutiRouterCableBuffer.BufferedBytes == DJEHUTI_ROUTER_CABLE_BUFFER_BYTES)
        {
            g_DjehutiRouterCableBuffer.ReadCursor =
                (g_DjehutiRouterCableBuffer.ReadCursor + 1) % DJEHUTI_ROUTER_CABLE_BUFFER_BYTES;
        }
        else
        {
            g_DjehutiRouterCableBuffer.BufferedBytes++;
        }
    }

    KeReleaseSpinLock(&g_DjehutiRouterCableBuffer.Lock, oldIrql);
}

static
VOID
DjehutiRouterCableReadBytes
(
...
)
{
    ...
    for (ULONG index = 0; index < ByteCount; ++index)
    {
        if (g_DjehutiRouterCableBuffer.BufferedBytes > 0)
        {
            Destination[index] = g_DjehutiRouterCableBuffer.Data[g_DjehutiRouterCableBuffer.ReadCursor];
            g_DjehutiRouterCableBuffer.ReadCursor =
                (g_DjehutiRouterCableBuffer.ReadCursor + 1) % DJEHUTI_ROUTER_CABLE_BUFFER_BYTES;
            g_DjehutiRouterCableBuffer.BufferedBytes--;
        }
...
--*/
{
    ...
    m_ulHostCaptureToneFrequency = 0;
    m_ulLoopbackCaptureToneFrequency = 0;
    m_dwHostCaptureToneAmplitude = 0;
    m_dwLoopbackCaptureToneAmplitude = 0;
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        DjehutiRouterCableReadBytes(m_pDmaBuffer + bufferOffset, runWrite);
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        DjehutiRouterCableWriteBytes(m_pDmaBuffer + bufferOffset, runWrite);
